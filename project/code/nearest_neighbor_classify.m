...
function predicted_categories = nearest_neighbor_classify(train_image_feats, training_labels, test_image_feats)
...
%Number of categories to choose when selecting the best matchces for each
%image
k = 9;
no_of_test_images = size(test_image_feats,1);
...
for nth_test_image = 1:no_of_test_images
    %Find the count of top 20 matching categories for each test image
    for nth_label = 1:no_of_tr_labels
        %Take the Top 20 labels of each test image
